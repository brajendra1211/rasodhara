...
export function HeroCarousel({ slides }: { slides: HeroSlide[] }) {
  ...
  return (
    ...
    >
      <div className="mx-auto grid w-full max-w-6xl grid-cols-1 items-center gap-8 px-4 py-10 sm:px-6 sm:py-16 md:grid-cols-2 md:gap-12">
        <div className="flex flex-col items-start gap-5">
          ...
          <h1 className="max-w-lg text-3xl leading-tight font-bold text-[#3f2d20] sm:text-5xl dark:text-zinc-50">
            {active.title}
          </h1>
        ...
        <div className="relative aspect-[4/3] w-full overflow-hidden rounded-3xl shadow-lg sm:aspect-[16/11]">
          {slides.map((slide, i) => (
            <div
              key={slide.title}
              aria-hidden={i !== index}
          ...
          {slides.length > 1 && (
            <>
              <button
                ...
                className="absolute left-3 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full bg-white/80 text-zinc-800 hover:bg-white"
              >
              ...
              <button
                ...
                className="absolute right-3 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full bg-white/80 text-zinc-800 hover:bg-white"
              >
                <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth={2}>
